...
class FS_WPML_Data_Converter
{
    ...
    private $lang_suffixes = [
        'ru_ru' => 'ru',
        'ru_ru_lower' => 'ru',  // ru_ru
        'ru_ru_upper' => 'ru',  // ru_RU
        'uk' => 'ua',
        'en_us' => 'en',
        'en_us_upper' => 'en',  // en_US
    ];

    /**
     * Get all possible suffix variants for a base suffix
     * 
     * @param string $base_suffix Base suffix like 'ru_ru'
     * @return array Array of possible suffix variants
     */
    private function get_suffix_variants($base_suffix)
    {
        $variants = [$base_suffix];
        
        // Add case variants
        $parts = explode('_', $base_suffix);
        if (count($parts) === 2) {
            // ru_ru -> ru_RU
    ...
    private function convert_meta_field($table, $id_column, $field)
    {
        ...
        foreach ($object_ids as $object_id) {
            ...
            // Delete old suffixed meta fields
            foreach ($this->lang_suffixes as $suffix => $lang_code) {
                $suffixed_key = $field . '__' . $suffix;
